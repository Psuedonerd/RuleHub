...
The output must be a readable explanatory article. It should explain the biological story, the major players, the key modeled events, what the model measures, and how to interpret likely plots. It must avoid exposing the reader to implementation details unless those details are needed for website integration outside the prose.
...
Give general but useful plot interpretation guidance. Explain what rising, falling, delayed, sustained, transient, or opposing curves mean for this model. Avoid promising a specific trajectory unless the model comments explicitly document it.
